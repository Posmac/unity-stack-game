...
const unityBuildPath = "../build/"; // Note: this makes the current working directory visible to all computers over the network.
...
app.use((req, res, next) => {
  ...
  if (enableWasmMultithreading && (path == "/" || path.includes(".js") || path.includes(".html") || path.includes(".htm"))) {
  ...
  } else if (path.includes(".data") || path.includes(".bundle") || path.endsWith(".unityweb")) {
  ...
  if (req.headers["cache-control"] == "no-cache" && (req.headers["if-modified-since"] || req.headers["if-none-match"])) {
